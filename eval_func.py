...
def eval_detection(gallery_dataset, gallery_dets, det_thresh=0.5, iou_thresh=0.5, labeled_only=False):
    """
...
def eval_search_cuhk(
    ...
    fname = f"TestG{gallery_size if not use_full_set else 50}"
    ...
    for i in range(len(query_dataset)):
        ...
        if cbgm:
            ...
            for img in imgs_cbgm:
                ...
                assert (query_roi - qboxes[0][:4]).sum() <= 0.001, "query_roi must be the first one in pboxes"
