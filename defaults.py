...
# Loss weight of box ReID on the source / target domain
_C.SOLVER.LW_BOX_REID = 1
_C.SOLVER.LW_BOX_REID_T = 0.1
# Loss weight of the domain-adaptive instance loss (overridden in configs/cuhk_sysu_da.yaml)
...
# -------------------------------------------------------- #
_C.MODEL = CN()
# SDH / SIA 模块中的特征融合系数，对应 models/resnet_scale.py 的 WEIGHT 全局量。
# 注意：不同权重训练时使用的值不同，加载错误的权重会得到远低于预期的结果。
#   FC (epoch_21, 目标域 PRW)  -> 1.0
#   FP (epoch_16, 目标域 CUHK) -> 0.2
_C.MODEL.WEIGHT = 0.2
_C.MODEL.RPN = CN()
...
# --------------------------------------------------------#
_C.PSEUDO_LABEL = CN ()
_C.PSEUDO_LABEL.CLUSTER = CN ()
_C.PSEUDO_LABEL.CLUSTER.EPS = 0.6 # 密度聚类邻域半径阈值
_C.PSEUDO_LABEL.CLUSTER.MIN_SAMPLES = 4 # 密度聚类核心点最小样本数
