...
from utils import MetricLogger, SmoothedValue, mkdir, reduce_dict, warmup_lr_scheduler
...
def evaluate_performance(model, gallery_loader, query_loader, device, use_gt=False, use_cache=False, use_cbgm=False):
    ...
    eval_search_func = (
        eval_search_cuhk
        if gallery_loader.dataset.name == "CUHK-SYSU" or gallery_loader.dataset.name == "CUHK-SYSU-COCO"
        else eval_search_prw
    )
    eval_search_func(
